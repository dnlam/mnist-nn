...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    # X_quad = np.multiply(theta,theta)
    # J1 = np.sum(np.sum(X_quad,axis=1),axis=0)
    # J1 = J1*lambda_factor /2
    # # compute loss
    # sum_n=0
    ...
    #                 s=s+np.exp(theta[l].dot(X[i])/temp_parameter)
    #             p_e = np.exp(np.dot(theta[j],X[i])/temp_parameter)/s
    #             sum_n += np.log(p_e)
    # J2= -sum_n/X.shape[0]
    #
    # return J1+J2
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    # grad matrix of theta
    delta_J = (-1/(temp_parameter*n))*((M-P).dot(X)) + lambda_factor*theta
    # update
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    predicted_Y = get_classification(X,theta,temp_parameter)
    return 1-(predicted_Y%3==Y).mean()
